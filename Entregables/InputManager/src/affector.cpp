...
{	
	if( mode != 0 )
	{
		...
		if( ( mode & COLOR_ACTIVE ) == COLOR_ACTIVE )
 		{
			particle->SetColor( (uint8)RangeRand((double)minr,(double) maxr ), (uint8)RangeRand( (double)ming, (double)maxg ), (uint8)RangeRand( (double)minb, (double)maxb ), particle->GetAlpha() );
		}
