...
bool Slider::init( const std::string name, const Vector2& position, const std::string& barImage, const std::string& pointerImage, const std::string& addImage, const std::string& addImagePushed, const std::string& substractImage, const std::string& substractImagePushed )
{
	...
	m_up = new Button();
	m_up->init( "sliderUp", Vector2( -(float)Image( addImage.c_str() ).GetWidth(), (float)m_position.y ), addImage.c_str(), addImagePushed.c_str() ); 
	m_up->setParent( this );

	m_down = new Button();
	m_down->init( "sliderDown", Vector2( (float)m_size.x , (float)m_position.y ), addImage.c_str(), addImagePushed.c_str() );
	m_down->setParent( this );

	m_posPointer = 50;
	m_pointer = new Button();
	m_pointer->init( "pointer", Vector2( (float)m_position.x , (float)m_position.y ), addImage.c_str(), addImagePushed.c_str() );
